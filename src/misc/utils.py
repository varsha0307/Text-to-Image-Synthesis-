...
def drawCaption(convas, captions, ixtoword, vis_size, off1=2, off2=2):
    ...
    #fnt = ImageFont.truetype("/app/AttnGAN-Pavan/img/FreeMono.ttf", 50)
    #fnt = ImageFont.load_default()
    # get a drawing context
    fnt = ImageFont.truetype("/app/AttnGAN-Pavan/img/arial.ttf", 50)
    ...
    for i in range(num):
        ...
        for j in range(num_attn):
            ...
            if (vis_size // att_sze) > 1:
                one_map = \
                    skimage.transform.pyramid_expand(one_map, 
        ...
        for j in range(seq_len + 1):
            if j < num_attn:
               ...
               # PIL_att = Image.fromarray(np.uint8(one_map))
                one_map = np.squeeze(one_map)  # Remove unnecessary dimensions
                if one_map.ndim == 3 and one_map.shape[-1] > 3:
                    one_map = one_map[:, :, :3]  # Use only the first 3 channels if needed
                PIL_att = Image.fromarray(np.uint8(one_map))  # Convert to Image
